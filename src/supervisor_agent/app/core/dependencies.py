...
from ..adapters.orchestration.handoff_policy import HandoffPolicyServiceImpl as HandoffPolicyService
...
async def get_handoff_service() -> HandoffPolicyService:
    return HandoffPolicyService()
...
async def get_checkpoint_store() -> RedisGraphCheckpointStore:
    return RedisGraphCheckpointStore()

async def get_orchestration_engine(
    invocation_service: DefaultA2AInvocationService = Depends(get_invocation_service),
    handoff_service: HandoffPolicyService = Depends(get_handoff_service),
    ...
    task_store: RedisTaskStore = Depends(get_task_store),
    checkpoint_store: RedisGraphCheckpointStore = Depends(get_checkpoint_store),
) -> Any:
    ...
    if engine_type == OrchestrationEngineType.BURR:
        ...
        return BurrAdapter(factory, checkpoint_store=checkpoint_store)
    
    graph_factory = LangGraphStateGraphFactory(
        invocation_service, handoff_service, progress_publisher, fact_service, task_store
    )
    return LangGraphAdapter(graph_factory, checkpoint_store=checkpoint_store)
...
async def get_worker_execution_service() -> WorkerExecutionService:
    ...
    # Engine Abstraction (Strategy Pattern) with checkpoint store
    checkpoint_store = RedisGraphCheckpointStore()
    engine_type = settings.orchestration_engine_setting
    if engine_type == OrchestrationEngineType.BURR:
        factory = BurrWorkflowFactory(invocation_service, handoff_service, publisher, fact_service, task_store)
        engine = BurrAdapter(factory, checkpoint_store=checkpoint_store)
    else:
        graph_factory = LangGraphStateGraphFactory(invocation_service, handoff_service, publisher, fact_service, task_store)
        engine = LangGraphAdapter(graph_factory, checkpoint_store=checkpoint_store)
