...
class ExecutionConsistencyCoordinator:
    ...
    async def complete_execution(self, session_id: str, task_id: str, result_data: Dict[str, Any]):
        ...
        Uses versioned CAS to prevent swarm state lost-update races.
        """
        # Extract swarm state if provided by the orchestrator
        swarm_state = result_data.pop("swarm_state", None)
        if swarm_state:
            swarm_ver = result_data.pop("swarm_state_version", None)
            try:
                await self.swarm_state_store.save_swarm_state(
                    session_id, swarm_state, expected_version=swarm_ver
                )
                logger.info("swarm_state_persisted", task_id=task_id, session_id=session_id)
            except ValueError as e:
                logger.warning("swarm_state_version_conflict", task_id=task_id, error=str(e))
    ...
    async def load_swarm_state_with_version(self, session_id: str):
        return await self.swarm_state_store.get_swarm_state_with_version(session_id)
