...
class SupervisorAgentService:
    ...
    async def handle_review_decision(self, request: Any) -> Tuple[bool, ReasonCode, Any]:
        ...
        if not session_id:
            return False, ReasonCode.SESSION_OWNERSHIP_MISMATCH, None
    ...
    async def _resolve_session_id_for_review(self, request: Any) -> Optional[str]:
        """
        Resolves session_id for HITL review decisions.
        Returns None if no session_id is found (caller handles the error path).
        """
    ...
    async def clear_session(self, session_id: str) -> bool:
        ...
        try:
            await self.conversation_store.delete_messages(session_id)
            await self.persistence_facade.strategy_factory.coordinator.swarm_state_store.delete_swarm_state(session_id)
    ...
    async def _process_approval(self, session_id: str, task_id: str, log: Any) -> Tuple[bool, ReasonCode, Any]:
        ...
        # Re-verify plan drift before enqueue (M1: close the verification-to-execution gap)
        drift_ok, drift_reason = await self._reverify_plan_drift(session_id, task_id, snapshot)
        if not drift_ok:
            log.warning("plan_drift_detected_before_enqueue", reason=drift_reason)
            await self.persistence_facade.cancel_task(session_id, task_id)
            return False, ReasonCode.PLAN_DRIFT_BLOCKED, None
    ...
    async def _reverify_plan_drift(self, session_id: str, task_id: str, snapshot: Any) -> Tuple[bool, str]:
        """
        Re-verifies the frozen plan against current agent registry right before enqueue.
        Closes the window between initial verification and actual execution.
        """
        verification = await self.read_facade.verify_snapshot(
            session_id, task_id,
        ...
        if not verification.is_allowed:
            return False, f"Drift detected: {verification.reason_code.value}"
        return True, ""
