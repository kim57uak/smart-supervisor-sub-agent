...
class SupervisorAgentService:
    ...
    async def _resolve_session_id_for_review(self, request: Any) -> str:
        ...
        Strictly relies on client-provided session_id as per UI-managed lifecycle rule.
        """
        # Rationale (Why): Per user rule, session_id must be generated by the UI. 
        # Removing Redis fallback to prevent session drift and ensure UI-managed lifecycle.
