...
이 폴더 문서는 이제 `Python 3.11+ + FastAPI + LangChain + LangGraph + A2A + Redis + MCP adapter runtime` 기준의 **엔터프라이즈급 구현 설계 문서**다.

## Enterprise Alignment

- **Hexagonal Architecture**: `ports`와 `adapters`를 통한 기술/도메인 분리.
- **CQRS**: `execution`, `persistence`, `read` 계층의 분리.
- **Decoupled Worker**: API와 무거운 실행 로직(Worker)의 분리.
- **Supervisor Consistency**: `src/supervisor-agent`의 구조 및 명명 규칙을 서브 에이전트에도 동일하게 적용.

## Reuse

- MCP registry/session/transport의 개념적 경계.
