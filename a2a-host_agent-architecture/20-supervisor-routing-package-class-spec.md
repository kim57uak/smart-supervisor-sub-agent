...
- **Reliable Invocation**: 모든 downstream 호출은 `DefaultA2AInvocationService`를 거치며 서킷 브레이커와 재시도를 자동 적용한다.
...
1. **Submission Phase (API Tier)**:
   ...
   - `SupervisorA2aRequestValidator`가 스키마 및 메서드 허용 여부 체크.
   - `ExecutionConsistencyCoordinator`가 `request_id` 기반 멱등성 선점.
   - `HitlGateService`가 플래닝 및 리뷰 필요성 평가.
   - 리뷰 불필요 시 `TaskQueueService`에 작업 적재 후 즉시 `STREAMING` 응답.

2. **Execution Phase (Worker Tier)**:
   - `worker.py`가 Redis Queue에서 작업 추출.
   - `SupervisorGraphExecutionService`가 실행 문맥 복원 및 LangGraph 실행.
   - `DefaultA2AInvocationService`가 downstream agent 호출.
   - `SupervisorProgressPublisher`가 Redis Stream에 실시간 진행 상태 발행.
   - `LlmSupervisorResponseComposeService`가 결과를 합성하고 최종 완료 처리.
...
- **`LangGraphStateGraphFactory`**: LangGraph 정의 및 컴파일.
...
- `consistency_coordinator.check_and_reserve_request(request_id) -> (bool, task_id)`
...
| Operation | Standard (Recommended) | Legacy (Supported) |
|---|---|---|
| send message | `message/send` | `SendMessage` |
| review decide | `tasks/review/decide` | `ReviewDecide` |
| get review | `tasks/review/get` | `GetReview` |
| get agent card | `agent/card` | `GetAgentCard` |
| cancel task | `tasks/cancel` | `CancelTask` |
| task events | `tasks/events` | `TaskEvents` |
| clear session | `session/clear` | `ClearSession` |
