...
Updated: 2026-04-29 (Independence & Enum Sync)

## Base Package

- `app` (at the root of each agent folder)
...
- **CQRS Patterns**:
  - `application/execution`: Command 및 전체 유즈케이스 흐름 제어.
  - `application/persistence`: 상태 변경(Write) 및 트랜잭션/일관성 관리.
  - `application/read`: 상태 조회(Read) 및 복합 쿼리/검증 로직.
...
- **Standard Env Naming**: `load_dotenv()`를 명시적으로 호출하여 `.env`의 `OPENAI_API_KEY`, `GOOGLE_API_KEY` 등을 시스템 환경 변수로 자동 등록한다.
...
- `main.py`: FastAPI 서버 진입점 (Sub-agent와 동일 구조).
- `worker.py`: 백그라운드 워커 진입점 (Sub-agent와 동일 구조).
